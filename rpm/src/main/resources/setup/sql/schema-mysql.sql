CREATE TABLE IF NOT EXISTS `inventory` (
	`id` int NOT NULL AUTO_INCREMENT,
	`gtin` varchar(14),
	`product_type` varchar(256),
	`style_code` varchar(64) NOT NULL,
	`color_code` varchar(64),
	`color_description` varchar(64),
	`color_type` varchar(64),
	`size_code` varchar(64),
	`brand` varchar(256),
	`gender` varchar(64),
	`fit` varchar(64),
	`price` DECIMAL(6,2),
	`price_map` DECIMAL(6,2),
	`collection` varchar(256),
	`description_short` TEXT,
	`description_long` TEXT,
    `status` int NOT NULL DEFAULT '1',
	`callouts` TEXT,
	`image_1` TEXT,
	`image_2` TEXT,
	`stock` int,
	`weight` DECIMAL(6,2),
	`supplier` varchar(64) NOT NULL,
	`source_input_name` varchar(256) NOT NULL,
	`source_input_id` int NOT NULL,
	PRIMARY KEY (`id`)
) ENGINE=Aria;

CREATE TABLE IF NOT EXISTS `products` (
	`id` int NOT NULL AUTO_INCREMENT,
	`gtin` varchar(14) NOT NULL UNIQUE,
	`sku` varchar(192) NOT NULL UNIQUE,
	`name` varchar(256) NOT NULL,
	`short_description` TEXT,
	`description` TEXT,
	`categories` TEXT,
	`url_key` varchar(192) NOT NULL,
	`price` DECIMAL(6,2) NOT NULL DEFAULT '0',
	`image` TEXT,
	`image_label` TEXT,
	`product_type` varchar(64),
	`product_online` int NOT NULL DEFAULT '0',
	`visibility` varchar(64),
	`attribute_set_code` varchar(256) NOT NULL DEFAULT 'Default',
	`additional_attribute_color` varchar(64),
	`additional_attribute_size` varchar(64),
    `additional_attribute_fit` varchar(64),
    `additional_attribute_gender` varchar(64),
    `additional_attribute_brand` varchar(256),
	`weight` DECIMAL(6,2) NOT NULL DEFAULT '0',
	`qty` int NOT NULL DEFAULT '0',
	`source_input_name` varchar(256) NOT NULL,
	`source_input_id` int NOT NULL,
	PRIMARY KEY (`id`)
) ENGINE=Aria;
